...
async fn parse_bond_row_inner(driver: &WebDriver, row: &WebElement, csv_filename: &Option<String>) -> Result<Option<BondListItem>> {
    ...
    if cells.len() >= 4 {
        ...
        if let Ok(name_div) = cells[0].find(thirtyfour::By::Css(".SecurityRow__showName_inlal")).await {
            bond_name = name_div.text().await.unwrap_or_default();
        }
        if let Ok(ticker_div) = cells[0].find(thirtyfour::By::Css(".SecurityRow__ticker_KMm7A")).await {
            bond_ticker = ticker_div.text().await.unwrap_or_default();
        }
        
        // cell 1 - Дата погашения
        if let Ok(date_div) = cells[1].find(thirtyfour::By::Css(".BondsTable__dateToClient_LjMTe")).await {
        ...
        let yield_text = if let Ok(money_elem) = cells[2].find(thirtyfour::By::Css(".Money-module__money_UZBbh")).await {
        ...
        let price_text = if let Ok(money_elem) = cells[3].find(thirtyfour::By::Css(".Money-module__money_UZBbh")).await {
            money_elem.text().await.unwrap_or_default()
        } else {
        ...
        println!("[DEBUG] Строка: {} | {} | {} | Доходность: {}% | Цена: {}₽", 
            bond_name, bond_ticker, maturity_date, 
            yield_to_maturity.map(|y| y.to_string()).unwrap_or_else(|| "N/A".to_string()),
            price.map(|p| p.to_string()).unwrap_or_else(|| "N/A".to_string()));
    }
    ...
    if windows.len() > 1 {
        ...
        let mut bond_result = BondListItem {
            ...
            name: format!("{} | {} | {}%", bond_name, maturity_date, final_yield.unwrap_or(0.0)),
            price,
        ...
        let skip_analysis = bond_result.maturity.as_ref().map(|maturity_str| {
        ...
        }).unwrap_or(false) || bond_result.price.map(|price| price - 1000.0 > 5.0).unwrap_or(false);
        
        // Анализируем облигацию через OpenCode (если срок > 1 года)
        if skip_analysis {
            println!("[DEBUG] Пропускаем анализ: срок погашения меньше 1 года - {}", bond_result.ticker);
        } else {
            println!("[DEBUG] Анализируем облигацию через OpenCode: {}", bond_result.ticker);
...
async fn collect_bond_details_inner(driver: &WebDriver) -> Result<BondDetails> {
    ...
    let tables = driver.find_all(thirtyfour::By::Css("table[data-qa-file=\"Table\"]")).await?;
    
    let mut details = BondDetails::default();

    // Обходим ВСЕ таблицы и ищем каждое поле по label
    for table in tables.iter() {
        let rows = table.find_all(thirtyfour::By::Css("tr[data-qa-type=\"uikit/table.tableRow\"]")).await?;
        ...
        for row in rows {
            ...
            if cells.len() >= 2 {
                ...
                let label_clean = label.trim().replace("\n", "").replace(" ", "").replace("\u{A0}", "").trim().to_string();
                ...
                if label_str.contains("Накопленныйкупонныйдоход") {
                    if details.accrued_coupon_income.is_none() {
                        let val = value_clean.replace("₽", "").replace(",", ".").parse::<f64>().ok();
    ...
    println!("[DEBUG]   Доходность к погашению: {}%", details.yield_to_maturity.map(|v| v.to_string()).unwrap_or_else(|| "N/A".to_string()));
    println!("[DEBUG]   Дата погашения облигации: {}", details.maturity.as_ref().map(|s| s.as_str()).unwrap_or("N/A"));
    println!("[DEBUG]   Дата выплаты купона: {}", details.next_coupon.as_ref().map(|s| s.as_str()).unwrap_or("N/A"));
    println!("[DEBUG]   Тип купона: {}", details.coupon_type.as_ref().map(|s| s.as_str()).unwrap_or("N/A"));
    println!("[DEBUG]   Накопленный купонный доход: {}₽", details.accrued_coupon_income.map(|v| v.to_string()).unwrap_or_else(|| "N/A".to_string()));
    println!("[DEBUG]   Величина купона: {}₽", details.coupon_amount.map(|v| v.to_string()).unwrap_or_else(|| "N/A".to_string()));
    println!("[DEBUG]   Номинал: {}₽", details.nominal.map(|v| v.to_string()).unwrap_or_else(|| "N/A".to_string()));
    println!("[DEBUG]   Количество выплат в год: {}", details.payments_per_year.map(|v| v.to_string()).unwrap_or_else(|| "N/A".to_string()));
    println!("[DEBUG]   Субординированность: {}", details.subordinated.as_ref().map(|s| s.as_str()).unwrap_or("N/A"));
    println!("[DEBUG]   Амортизация: {}", details.amortization.as_ref().map(|s| s.as_str()).unwrap_or("N/A"));
    println!("[DEBUG]   Для квалифицированных инвесторов: {}", details.for_qualified_investors.as_ref().map(|s| s.as_str()).unwrap_or("N/A"));
...
impl BondsCrawler {
    ...
    pub async fn check_page_available(&mut self) -> Result<bool> {
        ...
        match driver.find(thirtyfour::By::Css("tbody[data-qa-type=\"uikit/dataTable.tableBody\"]")).await {
            Ok(_) => {
                Ok(true)
            }
            Err(_) => {
    ...
    pub async fn collect_bonds(&mut self) -> Result<Vec<BondListItem>> {
        ...
        loop {
            ...
            let table_body = driver
                .find(thirtyfour::By::Css("tbody[data-qa-type=\"uikit/dataTable.tableBody\"]"))
                .await
                .map_err(|e| CrawlerError::SeleniumError(format!("Table not found: {}", e)))?;

            let rows = table_body
                .find_all(thirtyfour::By::Css("tr[data-qa-type=\"uikit/dataTable.tableLinkRow\"]"))
                .await
            ...
            for idx in 0..rows.len() {
                ...
                let row_selector = format!("tr[data-qa-type=\"uikit/dataTable.tableLinkRow\"]:nth-of-type({})", idx + 1);
                let row = match driver.find(thirtyfour::By::Css(&row_selector)).await {
                    Ok(r) => r,
                    Err(_) => {
                        // Пробуем найти через nth
                        let all_rows = table_body.find_all(thirtyfour::By::Css("tr[data-qa-type=\"uikit/dataTable.tableLinkRow\"]")).await?;
                        if idx < all_rows.len() {
                            all_rows[idx].clone()
                        } else {
                            continue;
            ...
            println!("[DEBUG] Страница {} обработана, всего собрано: {}", page_num, all_bonds.len());
    ...
    pub async fn run_crawl_loop(&mut self, duration_minutes: Option<u64>) -> Result<Vec<BondListItem>> {
        ...
        loop {
            match self.check_page_available().await {
                Ok(true) => {
                    match self.collect_bonds().await {
                        Ok(bonds) => {
