...
impl RabbitMQConsumer {
    ...
    where
        F: FnMut(String) -> futures::future::BoxFuture<'static, Result<(), CrawlerError>> + Send + Sync + 'static,
    {
        loop {
            println!("Connecting to RabbitMQ at: {}", self.connection_string);

            let connection = match Connection::connect(&self.connection_string, ConnectionProperties::default()).await {
                Ok(conn) => {
                    println!("✅ Connected to RabbitMQ");
                    conn
                }
                Err(e) => {
                    eprintln!("❌ Failed to connect to RabbitMQ: {}. Retrying in 5 seconds...", e);
            ...
            let channel = match connection.create_channel().await {
                Ok(chan) => chan,
                Err(e) => {
                    eprintln!("❌ Failed to create channel: {}. Retrying in 5 seconds...", e);
                    sleep(Duration::from_secs(5)).await;
                    continue;
                }
            };
                ...
                .await
            {
                Ok(q) => q,
                Err(e) => {
                    eprintln!("❌ Failed to declare queue: {}. Retrying in 5 seconds...", e);
                    sleep(Duration::from_secs(5)).await;
                    continue;
                }
            };
                ...
                .await
            {
                Ok(c) => c,
                Err(e) => {
                    eprintln!("❌ Failed to create consumer: {}. Retrying in 5 seconds...", e);
                    sleep(Duration::from_secs(5)).await;
                    continue;
                }
            };
